pipeline {  
    ...
    environment {  
        ...
        AWS_ACCOUNT_ID = '190535467756'  
        IMAGE_TAG = "${env.BUILD_NUMBER}"  
        IMAGE_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:${IMAGE_TAG}"  
        LATEST_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:latest"  
        DEPLOY_SERVER = '43.205.94.215'  
    }  
  
    stages {  
        stage('Checkout') {  
            steps {  
                git branch: 'main', url: 'https://github.com/manish-lakum/website-docker-demo.git'  
            }  
        }  
    ...
    steps {
        sh """
        aws ecr get-login-password --region $AWS_REGION | docker login --username AWS --password-stdin 190535467756.dkr.ecr.ap-south-1.amazonaws.com
        """
    }
}
